...
export default function MetaPageAnalyzer() {
  ...
  const runAnalysis = async (e) => {
    ...
    setLoading(true);
    try {
      const { data } = await api.post('/tools/meta-analyze', { page_url: pageUrl.trim() });
      setResult(data);
    } catch (err) {
  ...
  return (
    <div className="py-12 px-4">
      <div className="max-w-2xl mx-auto">
        <h1 className="text-3xl font-bold text-text-primary mb-2">Meta Page Analyzer</h1>
        <p className="text-text-muted mb-6">Enter your Facebook/Meta page URL for a quick health check and recommendations.</p>
        {!isAuthenticated && (
        ...
        {result && (
          <div className="glass-card p-5 animate-fade-in">
            ...
            <p className="text-text-muted text-sm">Page: {result.page_name || result.page_url}</p>
            {result.followers_estimate != null && <p className="text-text-muted text-sm mt-1">Followers (est.): {result.followers_estimate}</p>}
            {result.engagement_estimate != null && <p className="text-text-muted text-sm mt-1">Engagement (est.): {result.engagement_estimate}</p>}
            <p className="mt-3 text-accent font-medium">Score: {result.score}/100</p>
            {result.recommendations?.length > 0 && (
              <ul className="mt-3 text-text-muted text-sm list-disc list-inside">
