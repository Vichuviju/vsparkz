...
function ReportTemplates() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingTemplate) {
        await api.put(`/admin/report-templates/${editingTemplate.id}`, payload);
      } else {
        await api.post('/admin/report-templates', payload);
      }
      fetchTemplates();
      closeModal();
    } catch (err) {
  ...
  return (
    <div className="p-6">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
        <div>
          <h1 className="text-2xl font-semibold text-text-primary mb-2">Report Templates</h1>
          <p className="text-text-muted">Report templates and scheduled exports.</p>
        </div>
          ...
          <table className="w-full min-w-[500px]">
            <thead className="dark:bg-navy-800/50 bg-gray-50">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-sm font-medium">Scope</th>
              ...
              {templates.length === 0 ? (
                <tr>
                  <td colSpan={3} className="px-4 py-8 text-center text-text-muted">
                    No report templates yet. Add one to get started.
                  </td>
                </tr>
              ) : (
                templates.map((t) => (
                  <tr key={t.id} className="dark:hover:bg-navy-800/50">
                    ...
                    <td className="px-4 py-3">
                      <button type="button" onClick={() => openEdit(t)} className="text-primary-500 hover:underline text-sm mr-2">Edit</button>
                      <button type="button" onClick={() => setDeleteConfirm(t)} className="text-red-500 hover:underline text-sm">Delete</button>
                    </td>
                  </tr>
                ))
              )}
          ...
          <div className="glass-card rounded-2xl p-6 w-full max-w-md shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input type="text" value={form.scope} onChange={(e) => setForm((f) => ({ ...f, scope: e.target.value }))} className="w-full px-3 py-2 rounded-lg border dark:border-navy-600 dark:bg-navy-800 text-text-primary" placeholder="e.g. client, project" />
              </div>
              <div className="flex gap-2 pt-2">
                <button type="submit" disabled={saving} className="btn-primary px-4 py-2 rounded-lg text-sm">{saving ? 'Saving...' : (editingTemplate ? 'Update' : 'Create')}</button>
